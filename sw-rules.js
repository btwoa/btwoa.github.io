...
module.exports.config = {
  ...
  external: {
    stable: [
      ...
      /^https:\/\/cdn\.staticfile\.org\/npm\/[^/@]+\@[^/@]+\/[^/]+\/[^/]+$/,
    ],
    replacer: srcUrl => {
      if (srcUrl.startsWith('https://cdnjs.cloudflare.com')) {
        const url = new URL(srcUrl)
        return [
            srcUrl,
            `https://cdn.staticfile.org` + url.pathname,
            `https://cdn.bootcdn.net` + url.pathname,
...
module.exports.getSpareUrls = srcUrl => {
  if (srcUrl.startsWith("https://cdn.cbd.int")) {
    return {
      timeout: 3000,
      list: [srcUrl, `https://cdnjs.cloudflare.com/${new URL(srcUrl).pathname}`],
    };
  }
};
